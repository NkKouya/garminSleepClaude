...
非技術者へ「Python不要の `Setup.exe` 1個」で配るための手順。**2段階**でビルドする。
...
# 2) PyInstaller で GUI exe（one-dir）を生成
pyinstaller garminsleep.spec --noconfirm
#   → dist\GarminSleepReporter\GarminSleepReporter.exe（フォルダごと）

# 3) Inno Setup (https://jrsoftware.org/isdl.php) を入れて Setup.exe を生成
#    Inno Setup Compiler で installer.iss を開いて Build、または:
