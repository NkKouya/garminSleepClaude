...
### 毎朝の全自動化（GUIのボタンで登録）★おすすめ
`python setup_gui.py` を起動し、「4. 毎朝の自動実行」で実行時刻（既定 8:00）を
入力して［毎朝自動実行を登録］を押すだけです。内部で Windows タスクスケジューラに
登録されます（管理者権限は不要）。解除は［自動実行を解除］。

- 無人実行は `pythonw.exe` で行うため**コンソール窓は出ません**。実行ログは
  `logs/task.log` に追記されます。
- 「スリープから復帰して実行」「起動できなかった場合は次回起動時に実行」「バッテリー
  運用でも実行」は登録時に自動で設定されます（README下記の手動設定は不要）。
- 完了時のデスクトップ通知（トースト）は GUI のチェックで ON/OFF できます
  （`winotify` 導入時に有効。配信が「ブラウザで開く」の場合は自動表示されるため通知は省略）。
...
> GUIの［毎朝自動実行を登録］で登録した場合、以下の①②は自動で設定済みです。
> **③（OSの電源設定）だけ**は手動が必要なので、そこだけ実施してください。
...
## 配布用にビルドする（開発者向け・Phase 4）

非技術者へ「Python不要の `Setup.exe` 1個」で配るための手順。
PyInstaller（exe生成）→ Inno Setup（Setup.exe生成）の**2段階**だが、`build.ps1` で一括実行できる。

```powershell
# 事前に1回だけ: 依存とビルドツール
pip install -r requirements.txt
pip install pyinstaller
# Inno Setup も導入: https://jrsoftware.org/isdl.php

# 以降のリリースはこれ1コマンド（PyInstaller→Inno Setup を順に実行）
./build.ps1
#   → Output\GarminSleepReporter-Setup.exe を配布

./build.ps1 -Clean            # build/dist/Output を消してクリーンビルド
./build.ps1 -Version 1.0.1    # installer.iss の AppVersion を上げてからビルド
```

> コードを書き換えたら **必ず PyInstaller から作り直す**（`build.ps1` はそれを自動で行う）。
> Inno Setup は `dist\` を箱詰めするだけなので、ISCC 単体では中身が更新されない。
> 版を上げる時は `-Version` を使う（AppId は変えない＝アップデートとして認識される）。

手動で2段階を実行する場合:

```powershell
pyinstaller garminsleep.spec --noconfirm
#   → dist\GarminSleepReporter\GarminSleepReporter.exe（フォルダごと）
& "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer.iss
#   → Output\GarminSleepReporter-Setup.exe
```

配布物の動き:
- 統合エントリは `app.py`（引数なし=設定GUI / `--run`=無人実行 / `--register`等=自動実行登録）。
- インストール先は **`%LocalAppData%\Programs\GarminSleepReporter`（管理者不要・書込可能）**。
  設定・`sleep.db`・`output`・`logs`・トークンはこの exe フォルダ配下に保存される
  （`config.base_dir()` が exe フォルダを指す）。
- 自動実行タスクのコマンドは `GarminSleepReporter.exe --run`（窓なし、結果は `logs\task.log`）。
- 配布版は無料系バックエンド（`ollama` / `gemini` / `claude_cli`）が対象。`claude_api`
  はサイズ削減のため同梱していない（使う場合はソースから実行）。

利用者の流れ: `Setup.exe` を実行 → インストール → 設定画面で Garmin と分析エンジンを入力
→［今すぐ実行］で確認 →［毎朝自動実行を登録］。

## ログ
- `logs/sleep_report.log` : アプリのログ
- `logs/task.log`         : 無人実行（pythonw / exe `--run`）の標準出力/エラー
