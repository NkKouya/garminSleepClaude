...
タスクスケジューラ(毎朝8:00) → run_detailed_report.bat → detailed_report.py
  garmin_client.get_sleep_raw  : Garminの生睡眠データ取得
  intermediate.build_intermediate: 特徴量＋5分粒度タイムラインに圧縮
  analyzer.analyze_free_detailed : claude -p で時系列分析（サブスク＝API課金なし）
  → output/sleep_detail_*.txt / sleep_intermediate_*.json 保存
...
python detailed_report.py 2026-06-16   # 日付指定
python detailed_report.py --no-mail    # 送信せずファイル保存のみ
```
Garmin生データ取得 → 5分粒度に圧縮 → Claude が時系列分析 →
`output/sleep_detail_YYYY-MM-DD.txt`・`sleep_intermediate_YYYY-MM-DD.json` 保存 →
